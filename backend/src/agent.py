...
def load_content():
    """
    📖 Checks if shared-data/day4_tutor_content.json exists. 
    If NO: Creates the folder and generates it from DEFAULT_CONTENT.
    If YES: Loads it.
    """
    try:
        ...
        # Create shared-data folder if not present
        if not os.path.exists(shared_dir):
            os.makedirs(shared_dir)
            print("📁 Created 'shared-data' folder.")

        path = os.path.join(shared_dir, CONTENT_FILE)

        # Check if content file exists
        if not os.path.exists(path):
            print(f"⚠️ {CONTENT_FILE} not found. Generating data in shared-data...")
            with open(path, "w", encoding='utf-8') as f:
                json.dump(DEFAULT_CONTENT, f, indent=4)
            print("✅ Content file created successfully inside shared-data.")
...
async def select_topic(
    ...
    topic_id: Annotated[str, Field(description="The ID of the topic to study (e.g., 'loops', 'variables')")]
) -> str:
...
class TutorAgent(Agent):
    def __init__(self):
        ...
        super().__init__(
            instructions=f"""
            You are an Computer Tutor designed to help users master concepts like Loops and Variables.
